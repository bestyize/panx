...
- 常用创建动作以内嵌弹窗完成，避免打断当前文件工作台上下文。
- 新建文档弹窗支持直接填写文件名和正文内容，适合快速记录说明、备注或临时文本。

![Panx 新建文档](handbook/create_document.png)

### 分享设置弹窗

- 创建分享时先进入设置弹窗，可在生成链接前填写过期小时数和提取码。
- 当前被分享的文件或文件夹会直接展示在弹窗顶部，便于用户确认目标对象。

![Panx 分享设置](handbook/share_setting.png)

### 分享结果弹窗

- 生成分享后会显示结果弹窗，集中展示分享地址、复制入口和打开分享页入口。
- 剪贴板不可用时也保留手动复制路径，避免浏览器权限限制影响操作闭环。

![Panx 分享结果](handbook/share_panel.png)

### 已分享管理页

- 已分享页面集中展示名称、类型、访问次数、提取码状态和有效期，便于统一管理所有分享记录。
- 每条记录支持打开分享页、复制链接和取消分享，适合作为分享资产面板使用。

![Panx 已分享管理](handbook/share_page.png)
